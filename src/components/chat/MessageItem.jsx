...
const MessageItem = ({ message, currentUserId, senderRole, onReply, onChanged }) => {
  ...
  const react = async (emoji) => {
    ...
    onChanged({ ...message, reactions: data.reactions });
  };
  const edit = async () => {
    const content = window.prompt("Edit message", message.content);
    if (content == null) return;
    const data = await chatRequest(`/chat/messages/${message._id}`, { method: "PATCH", body: JSON.stringify({ content }) });
    onChanged(data.message);
  };
  const remove = async () => {
    if (!window.confirm("Delete this message?")) return;
    const data = await chatRequest(`/chat/messages/${message._id}`, { method: "DELETE" });
    onChanged(data.message);
  };
  return <article className={`chat-message${own ? " chat-message--own" : ""}`}>
    ...
    <div>
      ...
      <div className="chat-message-actions"><button onClick={() => onReply(message)}>Reply</button>{["👍", "❤️", "✅", "👀"].map(emoji => <button aria-label={`React ${emoji}`} key={emoji} onClick={() => react(emoji)}>{emoji}</button>)}{own && !message.deletedAt && <><button onClick={edit}>Edit</button><button onClick={remove}>Delete</button></>}{!own && <button onClick={() => chatRequest(`/chat/messages/${message._id}/report`, { method: "POST", body: JSON.stringify({ reason: "other", details: "Reported from message menu" }) })}>Report</button>}</div>
    </div>
  </article>;
};
