...
describe("CodeHub CLI web workflow", () => {
  test("Code menu shows clone, existing-project, branch, protection, and role details", () => {
    ...
    expect(screen.getByText("codehub clone https://codehub.sbs/Puskar/project-name")).toBeTruthy();
    expect(screen.getByText("codehub init https://codehub.sbs/Puskar/project-name")).toBeTruthy();
  ...
  test("copy buttons use the clipboard and Escape closes the accessible modal", async () => {
    ...
    await waitFor(() => expect(writeText).toHaveBeenCalledWith("codehub clone https://codehub.sbs/Puskar/project"));
  ...
  test("CLI documentation covers install, authentication, clone, protected branches, sync, errors, and logout", () => {
    ...
    expect(screen.getByText("npm install -g codehub-cli")).toBeTruthy();
