...
const NewPullRequest = () => {
  ...
  return <div className="pull-page"><Navbar /><main className="pull-container pull-container--form">
    ...
    {!state.loading && <form className="pull-form" onSubmit={submit}>
      <div className="pull-branches"><label>Base branch<select value={base} onChange={(event) => setBase(event.target.value)}>{branches.filter((branch) => branch.name !== compare).map((branch) => <option key={branch.name}>{branch.name}</option>)}</select></label><span aria-hidden="true">←</span><label>Compare branch<select value={compare} onChange={(event) => setCompare(event.target.value)}>{branches.filter((branch) => branch.name !== base).map((branch) => <option key={branch.name}>{branch.name}</option>)}</select></label></div>
