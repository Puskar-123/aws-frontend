...
const Home = () => {
  ...
  return <div className="public-home">
    <header className="home-nav"><BrandLogo /><nav aria-label="Public navigation"><a href="#features">Features</a><a href="#workflow">How it works</a><a href="#cli">CLI</a>{isAuthenticated ? <><Link to="/dashboard">Dashboard</Link><Link className="home-nav-primary" to="/profile">Profile</Link></> : <><Link to="/login">Sign in</Link><Link className="home-nav-primary" to="/signup">Get started</Link></>}</nav></header>
    <main>
      <section className="home-hero"><div className="home-hero-copy"><p className="home-eyebrow">A complete developer workflow</p><h1>Build, collaborate, and ship code with ContalSystem</h1><p className="home-lead">A modern version-control platform for repositories, branches, pull requests, issues, and team collaboration.</p><div className="home-actions">{isAuthenticated ? <Link className="home-button home-button--primary" to="/dashboard">Open dashboard</Link> : <><Link className="home-button home-button--primary" to="/signup">Get started</Link><Link className="home-button" to="/login">Sign in</Link></>}</div>{isLoading && <span className="home-auth-status" role="status">Checking session...</span>}</div>
        <div className="home-terminal" id="cli" aria-label="ContalSystem CLI example"><div><span /><span /><span /></div><pre><code><b>$</b> npm install -g codehub-sbs-cli{"\n"}<b>$</b> codehub login{"\n"}<b>$</b> codehub clone Owner/Repository{"\n"}<b>$</b> codehub branch feature/my-change</code></pre><p>Everything is ready to collaborate.</p></div>
      </section>
    ...
    <footer className="home-footer"><strong>ContalSystem</strong><p>Built with React, Node.js, Express, MongoDB, and AWS.</p></footer>
  </div>;
};
