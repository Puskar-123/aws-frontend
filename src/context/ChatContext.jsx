...
import{mergeMessages,normalizeMessageId}from"../utils/chatMessages";
...
export const ChatProvider=({children})=>{
  ...
  useEffect(()=>{
    ...
    value.on("conversation:joined",data=>{updateConversationPresence(data);if(data?.messages?.length)setMessages(current=>({...current,[data.conversationId]:mergeMessages(current[data.conversationId]||[],data.messages)}));});
    value.on("conversation:presence",updateConversationPresence);
    value.on("message:new",message=>{const key=normalizeMessageId(message.conversation);if(!key)return;setMessages(current=>({...current,[key]:mergeMessages(current[key]||[],message)}));if(openRef.current!==key)setConversations(current=>current.map(item=>String(item._id)===key?{...item,unreadCount:(item.unreadCount||0)+1,lastMessage:message}:item));});
